...
public class DFSVsrtRaw {
    ...
    public DFSVsrtRaw() {
        this.type = -1;
        this.data = new float[Config_Vsrt.MAX_FCT_CNT];
        this.timeStamp = -1l;
    }
     ...
     * @param ts
     */
    public DFSVsrtRaw(int type, byte[] bin, long ts){
        super();
        ...
        for(int i = 0 ; i < blength ; i+=4){
			...
			float[] data;
			data[i] = Float.intBitsToFloat(((buf[3]<<24)&0xFF000000) |((buf[2]<<16)&0xFF0000) |((buf[1]<<8)&0xFF00) |(buf[0]&0xFF));
			뒤집어서 저장
			cpu 메모리 access
			*/
        }
